...
<!-- ===================== RA_UNDO_CALM_CURVEDTEXT_V1 =====================
     Calms Undo/Redo only for curved-text UI interactions.
     - Suppresses 'object:added/removed' spam while sliding curve controls.
     - Commits ONE 'object:modified' when the gesture ends.
     - Leaves all other history behavior untouched.
     Paste at the very bottom of app.js.
======================================================================= -->
<script>
(() => {
  if (window.__RA_UNDO_CURVE_CALM_V1__) return;
  window.__RA_UNDO_CURVE_CALM_V1__ = true;

  const MATCH = /(curve|arc|bend|radius|spacing)/i;  // ids/names we consider “curved text” controls
  let editing = false;
  let endTimer = null;
  let c = null;
  let origFire = null;
  ...
  // Begin a “quiet” editing session: mute object add/remove events (history listeners won’t see the flood)
  function beginQuiet(){
    if (editing) return;
    c = C(); if (!c) return;
    editing = true;

    // Monkey-patch Fabric's event emitter for this canvas during the edit.
    origFire = c.fire;
    c.fire = function(type, opts){
      if (editing && /^object:(added|removed)$/.test(type)) {
        // swallow spammy add/remove while the curve sliders rebuild glyphs
        return this;
      }
      return origFire.call(this, type, opts);
    };
  }

  // Debounced end: restore event emitter and commit ONE undo step
  function scheduleEnd(){
    if (!editing) return;
    clearTimeout(endTimer);
    endTimer = setTimeout(endQuiet, 300);  // trailing debounce; tweak if you want snappier/longer
  }
  ...
  function endQuiet(){
    ...
    try{
      if (c && origFire) c.fire = origFire;     // restore emitter
      // Commit a single history snapshot by emitting one modified event.
      // (Your Undo recorder listens for 'object:modified' and will push one step.)
      const target = c?.getActiveObject?.() || null;
      if (c && typeof c.fire === 'function') c.fire('object:modified', { target });
      c?.requestRenderAll?.();
    }finally{
      origFire = null;
      c = null;
  ...
  // Wire all present/future curve-related inputs
  function wireInputs(root=document){
    $$('input,select,textarea,button', root).forEach(el=>{
      if (el.__raCurveWired) return;
      const id  = (el.id||'') + ' ' + (el.name||'') + ' ' + (el.getAttribute('aria-label')||'');
      if (!MATCH.test(id)) return;

      el.__raCurveWired = true;

      // While sliding/typing → quiet
      el.addEventListener('input',      beginQuiet,  {passive:true});
      el.addEventListener('mousedown',  beginQuiet,  {passive:true});
      el.addEventListener('touchstart', beginQuiet,  {passive:true});

      // When it settles → commit one step
      el.addEventListener('change',     scheduleEnd, {passive:true});
      el.addEventListener('mouseup',    scheduleEnd, {passive:true});
      el.addEventListener('touchend',   scheduleEnd, {passive:true});
      el.addEventListener('blur',       scheduleEnd, {passive:true});
    });
  }

  // Global pointer-up as a safety net (e.g., if user releases outside the control)
  ['mouseup','touchend','pointerup'].forEach(ev=>{
    window.addEventListener(ev, scheduleEnd, {passive:true});
  });

  // Initial + dynamic wiring
  function boot(){
    wireInputs(document);
    // Watch for UI re-renders; wire new curve controls automatically
    const mo = new MutationObserver(muts=>{
      for (const m of muts){
        m.addedNodes && m.addedNodes.forEach(n=>{
          if (n.nodeType===1) wireInputs(n);
        });
      }
    });
    mo.observe(document.documentElement, {childList:true, subtree:true});
