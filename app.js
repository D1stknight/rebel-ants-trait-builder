...
/* =========================================================
   DESKTOP In‑Flow Canvas Lock
   - Desktop devices only (pointer:fine & non-mobile UA)
   - Cancels any "position: fixed" centering on the canvas card
   - Removes #raCanvasGhost if present
   - Leaves Fabric canvas size/zoom untouched
   ========================================================= */
;(() => {
  'use strict';
  if (window.__RA_DESKTOP_INFLOW_LOCK__) return;
  window.__RA_DESKTOP_INFLOW_LOCK__ = true;

  // Detect real desktop device (don’t run on phones/tablets)
  const UA = navigator.userAgent || '';
  const isMobileUA =
    (navigator.userAgentData && navigator.userAgentData.mobile === true) ||
    /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Windows Phone|Opera Mini/i.test(UA);
  const isFinePointer = !!(window.matchMedia && window.matchMedia('(pointer: fine)').matches);
  const IS_DESKTOP = isFinePointer && !isMobileUA;
  if (!IS_DESKTOP) return;

  document.documentElement.classList.add('ra-desktop-inflow');

  function canvasCard() {
    const c = document.getElementById('c');
    if (!c) return null;
    // the fixed-centering code targets the canvas "card"/wrapper element
    return c.closest('.card, .panel, .box, .canvas-card, .content, .canvas-wrapper') || c.parentElement;
  }
  ...
  function normalize() {
    ...
    // remove the spacer that block uses
    const ghost = document.getElementById('raCanvasGhost');
    if (ghost && ghost.parentNode) ghost.parentNode.removeChild(ghost);

    // clear inline styles set by "fixed center" logic
    Object.assign(card.style, {
      position: '', left: '', top: '', width: '', zIndex: '', transform: '', margin: ''
    });

    // keep it centered/flowing via CSS
    ...
    // If the original block exposed an unfix hook, call it
    try { typeof window.__RA_UNFIX_CANVAS === 'function' && window.__RA_UNFIX_CANVAS(); } catch(_) {}

    // Prevent re-application attempts from re-setting position:fixed
    try { card.__raFixedCenterApplied = true; } catch(_) {}
  }

  // If that script tries to re-apply later, kill it immediately
  function watch() {
    const card = canvasCard();
    if (!card) return;
    const mo = new MutationObserver(() => {
    ...
    window.__RA_DESKTOP_INFLOW_MO__ = mo;
  }

  function boot() {
    normalize();
    watch();
    // Keep things sane while dragging the window smaller
    let raf = 0;
    const onResize = () => {
