...
/* ========== RA_PREVIEW_TAB_SAFE_BUTTON_v1 — add a separate safe viewer tab ========== */
(() => {
  if (window.__RA_PREVIEW_TAB_SAFE_BUTTON_V1) return;
  window.__RA_PREVIEW_TAB_SAFE_BUTTON_V1 = true;

  // Fabric canvas
  function C(){ return (window.canvas && typeof window.canvas.toDataURL==='function') ? window.canvas : null; }

  // Try to read HQ multiplier (HQ ×2, ×3…); default 2
  function getMultiplier(){
    try{
      const el = Array.from(document.querySelectorAll('*'))
        .find(x => /export\s*quality/i.test(x.textContent||''));
      const scope = el ? (el.parentElement || el) : document.body;
      const t = (scope.textContent||'').toLowerCase();
      const m = (t.match(/×\s*([1-8])/) || t.match(/\b([1-8])\b/) || [])[1];
      const n = parseInt(m,10);
      return (n>=1 && n<=8) ? n : 2;
    }catch(_){ return 2; }
  }

  // Write a tiny viewer page into the new tab
  function writeViewer(win, dataUrl){
    const html = `
<!doctype html><meta charset="utf-8"><title>Export</title>
<style>
  html,body{height:100%;margin:0;background:#0b0c10;}
  .wrap{position:fixed;inset:0;display:flex;align-items:center;justify-content:center}
  img#raImg{display:block;max-width:calc(100vw - 32px);max-height:calc(100vh - 32px);box-shadow:0 10px 28px rgba(0,0,0,.55);border-radius:8px}
  .hud{position:fixed;left:50%;bottom:10px;transform:translateX(-50%);color:#e5e7eb;opacity:.75;font:12px system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial}
</style>
<div class="wrap"><img id="raImg" alt="export"></div>
<div class="hud">Click image to toggle: Fit ↔ Actual size</div>
<script>
(function(){
  ...
  apply();
    ...
    catch(_){ try{ win.location.replace(dataUrl); }catch(__){} }
  }

  function openPreviewTab(){
    const c = C();
    if (!c){ alert('Canvas not ready yet. Try again.'); return; }

    // Open tab immediately (popup‑safe), then render
    const w = window.open('about:blank','_blank','noopener,noreferrer');
    if (!w){ alert('Popup blocked. Allow popups for this site.'); return; }
    try{ w.document.title='Export'; w.document.body.innerHTML='<div style="padding:12px;color:#e5e7eb;font:14px system-ui">Preparing image…</div>'; }catch(_){}

    let url='';
    try{
      url = c.toDataURL({ format:'png', multiplier:getMultiplier(), enableRetinaScaling:true });
    }catch(e){
      try{ w.close(); }catch(_){}
      alert('Export failed (CORS/security). Try a different image/host.');
      return;
    }
    writeViewer(w, url);
  }

  // Insert button next to “Download PNG”
  function ensureButton(){
    if (document.getElementById('raPreviewTabSafeBtn')) return;
    const dl = Array.from(document.querySelectorAll('button')).find(b=>/download\s*png/i.test(b.textContent||''));
    if (!dl || !dl.parentElement) return;

    const btn = document.createElement('button');
    btn.id = 'raPreviewTabSafeBtn';
    btn.type = 'button';
    btn.className = dl.className.replace(/\bprimary\b/g,'').trim() || 'btn';
    btn.style.marginLeft = '8px';
    ...
    dl.parentElement.insertBefore(btn, dl.nextSibling);
  }
