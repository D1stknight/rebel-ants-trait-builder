...
/* ==========================================================
   RA_TOKEN_LOADER_CHUMPZ_FIX_V1  — paste at the very bottom
   What it does:
   • Forces the “Load by token” button to use a contract you set
     (window.__RA_CONTRACT or ?contract=… in the URL).
   • Pulls image candidates from Reservoir.
   • Tries a CORS-safe dataURL first; if blocked, falls back to
...
(() => {
  ...
  function contractFromPage(){
    ...
    return (window.__RA_CONTRACT || q.get('contract') || q.get('c') || '').trim();
  }
  async function reservoirCandidates(contract, tokenId){
    const url = 'https://api.reservoir.tools/tokens/v7?media=true&tokens=' + encodeURIComponent(`${contract}:${tokenId}`);
    const r = await fetch(url, { headers:{ accept:'application/json' }, cache:'no-store' });
    if (!r.ok) return [];
    const j = await r.json();
    const t = j?.tokens?.[0]?.token || {};
  ...
  function placeTokenLabel(id){
    const c = C(); if (!c || !window.fabric) return;
    // remove any existing token label we added
    (c.getObjects()||[]).forEach(o => { if (o && o._kind === 'tokenId') c.remove(o); });
    const txt = new fabric.Text('#' + String(id), {
      originX:'center', originY:'top',
      left: c.getWidth()/2, top: 40,
      fontFamily: "Impact, Haettenschweiler, 'Arial Narrow Bold', sans-serif",
      fontSize: 52, fill:'#fff', stroke:'transparent', strokeWidth:0,
      editable:false, strokeUniform:true, paintFirst:'stroke', objectCaching:false
    });
    txt._kind = 'tokenId';
    c.add(txt); c.requestRenderAll();
  }
  async function useDataURLOnCanvas(dataURL){
    return await new Promise((res) => {
      fabric.Image.fromURL(dataURL, img => res(img), {}); // dataURL doesn’t need crossOrigin
    });
  }
  async function useURLOnCanvasNoCors(url){
    // no crossOrigin option → will taint canvas, but it SHOWS
    return await new Promise((res) => {
      fabric.Image.fromURL(url, img => res(img), {}); // intentionally no {crossOrigin:'anonymous'}
    });
  }
  ...
  async function loadTokenWithFallback(){
    const c = C(); const f = window.fabric;
    const status = $('tokenStatus');
    const idEl   = $('tokenIdInput');
    if (!c || !f){ alert('Canvas not ready'); return; }

    const tokenId = (idEl?.value || '').trim();
    if (!tokenId){ if (status) status.textContent = 'Enter a token ID.'; return; }

    const contract = contractFromPage();
    if (!contract){ if (status) status.textContent = 'Set window.__RA_CONTRACT or ?contract=…'; return; }

    if (status) status.textContent = 'Fetching token…';
    let urls = [];
    try { urls = await reservoirCandidates(contract, tokenId); } catch(_){}
    if (!urls.length){ if (status) status.textContent = 'No image URL found for this token.'; return; }

    if (status) status.textContent = 'Downloading image…';
    killOldBase(c);

    // 1) CORS‑safe path (preferred: export-friendly)
    for (const u of urls){
      try{
        const data = await fetchAsDataURL(u);
        const img  = await useDataURLOnCanvas(data);
        if (img){
          fitAndAddAsBase(img);
          placeTokenLabel(tokenId);
    ...
    // 2) No‑CORS fallback (view‑only; export will be tainted)
    const img = await useURLOnCanvasNoCors(urls[0]);
    if (img){
      fitAndAddAsBase(img);
      placeTokenLabel(tokenId);
      if (status) status.textContent = 'Loaded (view only: image host blocks CORS)';
      return;
    }
  ...
  // --- intercept the existing button in capture phase so our loader wins ---
  function wireOnce(){
    const btn = $('loadToken');
    if (!btn || btn.__raChFix) return;
    btn.__raChFix = true;
    btn.addEventListener('click', (e)=>{
      e.preventDefault(); e.stopImmediatePropagation(); e.stopPropagation();
      loadTokenWithFallback();
      return false;
    }, true);
  }
