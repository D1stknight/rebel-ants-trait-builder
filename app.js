...
/* =========================================================
   RA MOBILE — FIT IMAGE ON LOAD (V13)
   Purpose: On phones only, when a token/base loads (or you
   change the canvas px), set zoom so the whole image fits.
   - Works with v28 layout patch.
   - Does not run on desktop.
   - No changes to export; only the on-screen zoom.
   ========================================================= */
(function () {
  if (window.__raMobileFitV13) return;
  window.__raMobileFitV13 = true;

  const isMobile = () => window.matchMedia("(max-width: 900px)").matches;

  function findCanvasPanel() {
    // Find the panel whose header reads "Canvas"
    const heads = Array.from(document.querySelectorAll("h1,h2,h3,h4,h5"));
    const hdr = heads.find(h => /canvas/i.test(h.textContent || ""));
    return hdr ? (hdr.closest("section,div,article") || hdr.parentElement) : null;
  }

  function findCanvasSizeInput(panel) {
    // In the Canvas panel, grab the numeric input that looks like the px size
    if (!panel) return null;
    const inputs = Array.from(panel.querySelectorAll("input"));
    // choose the first input that parses to something in a plausible px range
    return inputs.find(el => {
      const v = parseInt((el.value || el.placeholder || "").replace(/[^\d]/g, ""), 10);
      return v >= 300 && v <= 2000;
    }) || null;
  }

  function findZoomInput(panel) {
    if (!panel) return null;

    // 1) direct hit: input that currently shows something like 100 or 100%
    const inputs = Array.from(panel.querySelectorAll("input"));
    let cand = inputs.find(el => {
      const raw = (el.value || el.placeholder || "").toString().trim();
      return /^\d{2,3}%?$/.test(raw); // 50..150 or 50%..150%
    });
    if (cand) return cand;

    // 2) sometimes the zoom is a spinbutton element (accessibility)
    cand = panel.querySelector('[role="spinbutton"]');
    if (cand) return cand;

    // 3) fallback: look between a "-" and "+" button
    const btns = Array.from(panel.querySelectorAll("button, [role='button']"));
  ...
  function canvasPixels(panel) {
    const inp = findCanvasSizeInput(panel);
    const v = parseInt(inp ? (inp.value || inp.placeholder || "700") : "700", 10);
    return (isFinite(v) && v > 0) ? v : 700;
  }

  function canvasShellWidth() {
    // Width available to the canvas area in-flow on mobile
    // Prefer the shell from v28 if present
    const shell =
      document.querySelector('[data-ra-flow="canvas-shell"]') ||
      document.querySelector('[data-ra-canvas-shell]') ||
      document.querySelector("#raCanvasShell") ||
      document.body;

    // Leave some breathing room so it doesn't touch edges
    const margin = 24;
    const width = Math.max(0, (shell.clientWidth || window.innerWidth) - margin);
    // Safeguard: never bigger than viewport width
    return Math.min(width, window.innerWidth - margin);
  }

  function calcFitPercent() {
    const panel = findCanvasPanel();
    const px = canvasPixels(panel);           // e.g., 700 / 900 / 1024 / 1200
    const avail = canvasShellWidth();         // actual width we can use on phone
    let pct = Math.floor((avail / px) * 100); // convert to %
    // Keep it sensible
  ...
  function applyZoom(pct) {
    const panel = findCanvasPanel();
    const zi = findZoomInput(panel);

    if (zi) {
      if (zi.getAttribute("role") === "spinbutton") {
        zi.setAttribute("aria-valuenow", String(pct));
        // many spinbuttons mirror their value in text
        zi.textContent = pct + "%";
        zi.dispatchEvent(new Event("input",  { bubbles: true }));
        zi.dispatchEvent(new Event("change", { bubbles: true }));
      } else {
        zi.value = String(pct);
        zi.dispatchEvent(new Event("input",  { bubbles: true }));
        zi.dispatchEvent(new Event("change", { bubbles: true }));
      ...
      // If we can't find the zoom input, do nothing else—avoid touching desktop/other logic.
    }
  }

  function fitNow() {
    if (!isMobile()) return;
    // Delay slightly so the base image & panel layout settle
    requestAnimationFrame(() => setTimeout(() => applyZoom(calcFitPercent()), 60));
  }
  ...
  function install() {
    ...
    window.addEventListener("orientationchange", () => setTimeout(fitNow, 200));
    window.addEventListener("resize", () => setTimeout(fitNow, 120));

    // Re-fit after likely actions: "Load", "Load by Token", canvas px buttons
    const panel = findCanvasPanel();
    if (panel) {
      const buttons = Array.from(document.querySelectorAll("button, [role='button']"));
      buttons.forEach(b => {
        const t = (b.textContent || "").toLowerCase();
        if (/^load\b/.test(t) || /load by token/.test(t) || /^reset$/.test(t) || t === "-" || t === "+") {
          b.addEventListener("click", () => setTimeout(fitNow, 140));
        }
      });

      // Also watch the “Loaded 👍” badge/text if present
      const loadedNode = Array.from(document.querySelectorAll("*"))
        .find(n => /\bLoaded\b/.test(n.textContent || ""));
      if (loadedNode) {
        const mo = new MutationObserver(() => setTimeout(fitNow, 140));
        mo.observe(loadedNode, { childList: true, subtree: true, characterData: true });
      }

      // Re-fit when the px input changes directly
      const pxInput = findCanvasSizeInput(panel);
      if (pxInput) {
        pxInput.addEventListener("input", () => setTimeout(fitNow, 100));
        pxInput.addEventListener("change", () => setTimeout(fitNow, 100));
      }
    }
  }
