...
/* ===== RA_FOOTER_AND_WATERMARK_GUARD_v1 — Only show footer/watermark when base image exists ===== */
(() => {
  function C() {
    return (window.canvas && window.canvas.upperCanvasEl) ? window.canvas : null;
  }
  ...
  function enforceFooterAndWM() {
    ...
    // If base exists, let the normal logic create/position footer/watermark
  }
  function wireGuard() {
    const c = C();
    if (!c) { setTimeout(wireGuard, 100); return; }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', wireGuard, { once:true });
  } else {
    wireGuard();
  }
})();
