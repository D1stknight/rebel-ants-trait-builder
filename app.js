...
/* ===== RA_FOOTER_AND_WATERMARK_GUARD_v1 — Only show footer/watermark when base image exists (Safe Version) ===== */
(function RA_FOOTER_AND_WATERMARK_GUARD_SAFE(){
  function C() {
    return (window.canvas && window.canvas.upperCanvasEl) ? window.canvas : null;
  }
  function getFooter() {
    const c = C();
    return c && (c.getObjects?.() || []).find(o =>
      o._raBrandFooter ||
      o._raFooterId === 'footer-group' ||
      (typeof o.text === 'string' && /powered\s+by/i.test(o.text))
    );
  ...
  function getBase() {
    const c = C();
    return c && (c.getObjects?.() || []).find(o => o._isBase && !o._isBgRect);
  }
  function remove(obj) {
    const c = C();
    if (obj && c) try { c.remove(obj); c.requestRenderAll(); } catch(_) {}
  }
  function enforceFooterAndWM() {
    const c = C(); if (!c) return;
    try {
      const base = getBase();
      const footer = getFooter();
      const wm = getWatermark();
      // If no base image, remove both footer and watermark
      if (!base) {
        if (footer) remove(footer);
        if (wm) remove(wm);
      }
      // If base exists, do nothing (let your normal logic run)
    } catch(err) {
  ...
  function wireGuard() {
    const c = C();
    if (!c) { setTimeout(wireGuard, 200); return; }
    if (!c.__footerWmGuardBound) {
      c.__footerWmGuardBound = true;
      c.on('after:render', enforceFooterAndWM);
      c.on('object:added', enforceFooterAndWM);
      c.on('object:removed', enforceFooterAndWM);
      c.on('object:modified', enforceFooterAndWM);
      // Also on canvas resize
      try {
        const el = c.getElement ? c.getElement() : c.upperCanvasEl;
        new ResizeObserver(enforceFooterAndWM).observe(el);
      } catch(_) {}
      // Also on key app events
      ['ra-wm-recalc', 'ra-holder-update', 'ra-collection-change'].forEach(ev =>
        document.addEventListener(ev, enforceFooterAndWM)
      );
      // Initial pass
      enforceFooterAndWM();
    }
  }
  // Wait until DOM and app are fully loaded
  function waitForCanvasAndWire() {
    if (!window.canvas || !window.fabric) {
      setTimeout(waitForCanvasAndWire, 200);
      return;
    }
    wireGuard();
  }
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', waitForCanvasAndWire, { once:true });
  } else {
    waitForCanvasAndWire();
  }
})();
