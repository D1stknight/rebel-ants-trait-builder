...
if (isPermanent) {
  ...
  // 🔁 Deep‑swap any old (yellow) stamps inside this overlay group too
  swapOldStampsInGroup(group);
...
(function () {
  ...
  const hook = setInterval(() => {
    const c = window.canvas;
    if (c && c.on) {
      c.on('object:added', swapAllWatermarks);
      c.on('after:render', swapAllWatermarks);
      clearInterval(hook);
      swapAllWatermarks();
    }
  }, 200);
